...
export const get_collaboration = async ({ team_id, month }: { team_id: string, month?: Date }) => {
    // If month is undefined, use the current month
    const targetDate = month || new Date();
    ...
    const data = await formatCollaborationData(kudos);
    return data;
}

const formatCollaborationData = async (kudosData: any[]) => {
    const userStats: Record<string, any> = {};
    const users = await get_all_users();
    ...
    kudosData.forEach((kudos) => {
        ...
        if (!userStats[to_user_id]) {
            userStats[to_user_id] = {
                ...
                kudosReceived: 0,
                kudosGiven: 0,
                kudosReceivedCount: 0, // Track the number of times kudos are received
                kudosGivenCount: 0, // Track the number of times kudos are given
                categories: {},
                name: users.find((user) => user.id == to_user_id)?.name,
            };
        }
        ...
        // Add to kudosReceived for the receiver
        userStats[to_user_id].kudosReceived += kudosValue;
        userStats[to_user_id].kudosReceivedCount += 1;

        // Add to kudosGiven for the sender
        userStats[from_user_id].kudosGiven += kudosValue;
        userStats[from_user_id].kudosGivenCount += 1;
    ...
    // Calculate averages and highlights (top 2 categories)
    const leaderboardData = Object.values(userStats).map((user) => {
        const avgKudosReceived = user.kudosReceived / user.kudosReceivedCount || 0;
        const avgKudosGiven = user.kudosGiven / user.kudosGivenCount || 0;

        // Combine the averages (you can decide how to combine them, e.g., average of both)
        const avgKudos = (avgKudosReceived + avgKudosGiven) / 2;
        ...
        return {
            ...
            avgKudosReceived,
            avgKudosGiven,
            avgKudos, // Combined average kudos (received and given)
            topCategory: sortedCategories[0],
            highlights: sortedCategories,
        };
    });

    // Sort by average kudos (you can decide the sort order, ascending/descending)
    leaderboardData.sort((a, b) => b.avgKudos - a.avgKudos);  // Sort descending by avgKudos

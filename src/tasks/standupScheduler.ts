    ...
    try {
      ...
      for (const config of configs as (StandupConfigsModel & {
        ...
        for (const member of config.Team.TeamMembers) {
          ...
          if (reminderTime === userCurrentTime && config.reminder_days.includes(userCurrentDay)) {
          ...
          const userDueTime = moment.tz(`${userCurrentDay} ${config.due_time}`, "dddd HH:mm", userTimeZone)
